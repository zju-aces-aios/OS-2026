...
|---:|---|---|---:|---:|---:|
| 1 | 朴素 baseline | 设备 | 64×64×64 | 40.590 ms |  |
| 2 | HVX 内积 (A * B^T) | 设备 | 64×64×64 | 45.920 ms  |  |
| 3 | HVX 外积 (A * B) | 设备 | 64×64×64 | 33.392 ms |  |
| 4 | 朴素 baseline | 设备 | 256×256×256 | 472.255 ms  |  |
| 5 | HVX 内积 (A * B^T) | 设备 | 256×256×256 |  176.192 ms |  |
| 6 | HVX 外积 (A * B) | 设备 | 256×256×256 | 124.732 ms |  |
| 7 | 朴素 baseline | 设备 | 512×512×512 |  3595.342 ms |  |
| 8 | HVX 内积 (A * B^T) | 设备 | 512×512×512 | 891.978 ms  |  |
| 9 | HVX 外积 (A * B) | 设备 | 512×512×512 | 671.919 ms |  |
| 10 | 朴素 baseline | 设备 | 88×99×66 | 60.319 ms |  |
| 11 | HVX 内积 (A * B^T) | 设备 | 88×99×66 | 44.611 ms |  |
| 12 | HVX 外积 (A * B) | 设备 | 88×99×66 | 37.487 ms |  |
分析要点：

1. 内积 vs 外积在复用、访问与指令上的差异
- 数据复用: 内积(ijk, B^T)同时复用 A 的行段与 B^T 的行段；外积(ikj)复用 A[i,l] 的广播以一次更新 C 的一整段。
- 访问模式: 内积对 A、B^T 都是顺序读取，C 按标量位置写回；外积对 B 的行段顺序读取，在寄存器中完成 k 维累加后对 C 的向量块一次性写回，写流量更低。
- 向量指令: 内积需要“乘加+归约”得到标量 C[i,j]；外积采用“广播+乘加”直接更新 C 的 32-lane 子向量，不需要额外归约。

2. 关键 HVX 指令详解与性能收益
- Q6_V_vsplat_R: 将标量 A[i,l] 广播到 32 个 lane，支撑外积一次处理 32 个 C 元素，摊薄标量加载成本。
- Q6_Vqf32_vmpy_VsfVsf: 32 路向量乘（QF32 中间结果），显著提升吞吐并降低累加溢出风险。
- Q6_Vqf32_vadd_Vqf32Vqf32: 32 路向量加，在寄存器中完成 k 维累加，减少对 C 的中间读写。
- Q6_V_vror_VR: 向量循环右移，内积路径可用其做树形归约，在 O(log 32) 步内完成规约。
- Q6_Vsf_equals_Vqf32: QF32→SF 转换与饱和，确保把寄存器累加结果正确写回 float 缓冲，避免数值失真。

3. 尾部、对齐、缓存与带宽优化建议
- 尾部: 令 vec_limit=(n/HVX_LENGTH)*HVX_LENGTH 处理整向量；j∈[vec_limit,n) 标量补尾，保证覆盖。
- 对齐: 目标 128B 对齐；无法保证时用 memcpy 将内存段装入 HVX_Vector，避免对齐陷阱。
- 缓存/分块: 按 j、k 分块提升 L2/VTCM 命中；外积在寄存器中做完 k 维累加再一次性写回 C，降低写放大。
- 带宽: 外积优先顺序流式读取 B 行段；内积优先使用 B^T 保证顺序访问，并尽量减少对 C 的反复读写。

延伸讨论（可选）：

- l2fetch: 预取即将访问的 B 块（例如 j 分块后，在 l 循环中预取 &B[l*n + j] 的后续行段），隐藏 DRAM 延迟；注意 stride、批量与在途上限。
- 其他优化: 多寄存器并行累加多个 C 向量块、双缓冲隐藏加载延迟、按 i 维多线程切分、在内积路径用旋转归约替代标量归约以提升归约吞吐。
