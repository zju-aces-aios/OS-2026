...
static inline float hvx_reduce_sum(HVX_Vector ve c) {
    for (int shift = 16; shift >= 1; shift /= 2) {
        HVX_Vector tmp = Q6_V_vror_VR(vec, shift * sizeof(float));
        vec = Q6_Vqf32_vadd_Vqf32Vqf32(vec, tmp);
    }
    HVX_Vector vec_sf = Q6_Vsf_equals_Vqf32(vec);
    float result;
    memcpy(&result, &vec_sf, sizeof(float));
...
static inline void matmul_ijk(float *restrict input_matrix1,
    ...
    for (int i = 0; i < m; i++) {
        ...
        for (int j = 0; j < k; j++) {
            tmp_n = n;
            HVX_Vector vec_a = Q6_V_vsplat_R(float_to_bits(input_matrix1[i * k + j]));
            for (int s = 0; s < section; s++) {
                HVX_Vector vec_b = Q6_V_vzero();
                if (tmp_n > 32) {
                    memcpy(&vec_b, &input_matrix2[j * n + s * 32], 32 * sizeof(float));
                    tmp_n -= 32;
                } else {
                    memcpy(&vec_b, &input_matrix2[j * n + s * 32], tmp_n * sizeof(float));
                }
                HVX_Vector mul = Q6_Vqf32_vmpy_VsfVsf(vec_a, vec_b);
                vec[s] = Q6_Vqf32_vadd_Vqf32Vqf32(vec[s], mul);
            }
        }

        tmp_n = n;
        for (int s = 0; s < section; s++) {
            HVX_Vector res = Q6_Vsf_equals_Vqf32(vec[s]);
            if (tmp_n > 32) {
...
static inline void matmul_ikj_transposed_b(float *restrict input_matrix1,
    ...
    for (int i = 0; i < m; i++) {
		for (int j = 0; j < n; j++) {
            HVX_Vector vec = Q6_V_vzero();
            int l = 0;
            for (; l + 31 < k; l += 32) {
                HVX_Vector vec_a, vec_b;
                memcpy(&vec_a, &input_matrix1[i * k + l], 128);
                memcpy(&vec_b, &input_matrix2[j * k + l], 128);
                HVX_Vector mul = Q6_Vqf32_vmpy_VsfVsf(vec_a, vec_b);
                vec = Q6_Vqf32_vadd_Vqf32Vqf32(vec, mul);
            }
            int rem = k - l;
            if (rem > 0) {
                HVX_Vector vec_a = Q6_V_vzero();
                HVX_Vector vec_b = Q6_V_vzero();
                memcpy(&vec_a, &input_matrix1[i * k + l], rem * sizeof(float));
                memcpy(&vec_b, &input_matrix2[j * k + l], rem * sizeof(float));
                HVX_Vector mul = Q6_Vqf32_vmpy_VsfVsf(vec_a, vec_b);
                vec = Q6_Vqf32_vadd_Vqf32Vqf32(vec, mul);
            }
            float sum = hvx_reduce_sum(vec);
            output[i * n + j] = sum;
		}
	}
